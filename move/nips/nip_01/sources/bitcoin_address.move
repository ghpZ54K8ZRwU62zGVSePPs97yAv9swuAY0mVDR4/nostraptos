...
module nip_01_addr::bitcoin_address {
    ...
    public fun to_aptos_address(addr: &BitcoinAddress): address{
        assert!(!is_empty(addr), ErrorInvalidAddress);
        let hash =  aptos_std::aptos_hash::blake2b_256(addr.bytes);
    ...
    public fun random_address_for_testing(): BitcoinAddress {
        let bytes = std::bcs::to_bytes(&0x111);
        BitcoinAddress{
            bytes
        }
    }
